...
public class NavigationTagV2 extends TagSupport
{
	...
	public int doStartTag() throws JspException
	{
		...
		try
		{
			writer.print("<div class='block_pager'>");
			
			//是否第一页(如果是第一页则不会显示上一页)
			if (onePage.isHasPre())
			{
				...
				writer.print("<a class='prev' href=\"" + preUrl + "\">上一页</a>");
				if (onePage.getIndex() > 2 && onePage.isHasNext())
				{
					writer.print("<a class='prev' href=\"" + append(url, "pn", 1) + "\">1</a>");
				}
			}
			//是否最后一页(如果是最后一页则不显示下一页)
			if (onePage.isHasNext())
			{
				...
				writer.print("<a class='next' href=\"" + nextUrl + "\">下一页</a>");
				if (onePage.getContext().getPageCount() - onePage.getIndex() > 2)
				{
					writer.print("<li><a href=\"" + append(url, "pn", onePage.getContext().getPageCount()) + "\">" + onePage.getContext().getPageCount() + "</a></li>");
				}
			}
			writer.print("<div class='pages'><ul>");
			...
			for (int i = 1; i <= number && startIndex <= onePage.getContext().getPageCount(); startIndex++, i++)
			{
				...
				{
					//是否当前页面
					writer.print("<li class='current'><a href='#'>"+startIndex+"</a></li>");
					continue;
				}
				...
				writer.print("<li><a href=\"" + pageUrl + "\">" + startIndex + "</a></li>");
			}
		//	writer.print("&nbsp;(共" + onePage.getContext().getTotal() + "条记录)<br/></div>");
			writer.print("</ul></div></div>");
