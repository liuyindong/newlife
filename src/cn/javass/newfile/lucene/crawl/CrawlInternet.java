...
public class CrawlInternet extends Thread
{
	private static Logger logger = LoggerFactory.getLogger(CrawlInternet.class);
...
//	private static final String DOWN_DATE_TIME = "yyyy-MM-dd HH";
	...
	public CrawlInternet(NewService newServices)
	{
	...
	public static void main(String[] args)
	{
		try
		{
			CrawlInternet.downNewMsg(downInternet);
		/*	for (int i = 2; i < 10; i++)
			{
				CrawlInternet.downNewMsg(downInternet + "list_" + i + ".html");
			}*/
	...
	//	boolean isTaday = false;
		...
		for (Element message : msgList)
		{
			...
			if (a.size() == 3)
			{
				...
				//		if (DateUtil.jisuan(DateUtil.stringToTime(downDate, DOWN_DATE_TIME), DateUtil.timeToString(new Date(), DOWN_DATE_TIME), DOWN_DATE_TIME) == 0)
				//		{
