...
(function($)
{
	...
	$.fn.validationEngine = function(settings)
	{
		...
		{
			$.validationEngine
					.debug("Validation engine rules are not loaded check your external file");
		}
		settings = jQuery.extend(
		{
			...
			promptPosition : "topRight", // OPENNING BOX POSITION,
											// IMPLEMENTED: topLeft, topRight,
											// bottomLeft, centerRight,
											// bottomRight
			success : false,
			failure : function()
			{
		...
		$.validationEngine.ajaxValidArray = new Array(); // ARRAY FOR AJAX:
															// VALIDATION MEMORY
		...
		{ // Validating Inline ?
			...
			{ // NEEDED FOR THE SETTING returnIsValid
				...
				$(this).find("[class*=validate]").not("[type=checkbox]").bind(
						settings.validationEventTriggers, function(caller)
						{
							_inlinEvent(this);
						})
		...
		{ // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT
			// READY
	...
	$.validationEngine =
	{
		...
		{ // EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
			...
			{
				$.validationEngine
						.debug("This field have no ID attribut( name & class displayed): "
								+ $(caller).attr("name")
								+ " "
								+ $(caller).attr("class"))
			}
			...
			if ($.validationEngine.isError == true)
			{
				...
				($("div." + linkTofield).size() == 0) ? $.validationEngine
						.buildPrompt(caller, promptText, "error")
			...
			function radioHack()
			{
				...
				{ // Hack for radio/checkbox group button, the validation go
					// the first radio/checkbox of the group
				...
				if (!ajaxisError)
				{
					$
							.ajax(
							{
								...
								url : postfile,
								async : true,
								data : "validateValue=" + fieldValue
										+ "&validateId=" + fieldId
										+ "&validateError=" + customAjaxRule,// +extraData,
								beforeSend : function()
									...
									if ($.validationEngine.settings.allrules[customAjaxRule].alertTextLoad)
									{
										...
										if (!$("div." + fieldId + "formError")[0])
										{
											return $.validationEngine
													.buildPrompt(
															ajaxCaller,
															$.validationEngine.settings.allrules[customAjaxRule].alertTextLoad,
															"load");
										} else
										{
											$.validationEngine
								...
								{ 
								//	alert(obj2str(data));
									data = eval("(" + data + ")"); // GET JSON
									ajaxisError = data.jsonValidateReturn.result;
									customAjaxRule = data.jsonValidateReturn.validateError;
									ajaxCaller = $("#"+ data.jsonValidateReturn.validateId)[0];
									fieldId = ajaxCaller;
									...
									{ 
										...
										{ // Add ajax error to stop submit
											$.validationEngine.ajaxValidArray[ajaxErrorLength] = new Array(
													2);
			...
			{ // VALIDATE FIELD MATCH
				...
				if ($(caller).attr('value') != $("#" + confirmField).attr(
						'value'))
				{
					$.validationEngine.isError = true;
			...
			{ // VALIDATE CHECKBOX NUMBER
				...
				if (groupSize > nbCheck)
				{
					$.validationEngine.showTriangle = false;
					$.validationEngine.isError = true;
					promptText += $.validationEngine.settings.allrules["maxCheckbox"].alertText
							+ " "
							+ nbCheck
							+ " "
							+ $.validationEngine.settings.allrules["maxCheckbox"].alertText2
							+ "<br />";
				}
			}
			...
			if ($.validationEngine.settings.ajaxSubmit)
			{
				...
				$
						.ajax(
						{
							...
							error : function(data, transport)
							{
								$.validationEngine.debug("error in the ajax: "
							...
							success : function(data)
							{
								...
								{ // HOUSTON WE GOT A PROBLEM (SOMETING IS NOT
									// VALIDATING)
									data = eval("(" + data + ")");
									if (!data.jsonValidateReturn)
									{
										$.validationEngine
												.debug("you are not going into the success fonction and jsonValidateReturn return nothing");
									}
									...
									for (index = 0; index < errorNumber; index++)
									{
										...
										$.validationEngine.buildPrompt(fieldId,
												promptError, type);
									}
								}
							}
		...
		debug1 : function(error)
		{
			...
			$(".debugError").append(
					"<div class='debugerror'>" + error + "</div>");
		},
		...
		{ // FORM SUBMIT VALIDATION LOOPING INLINE VALIDATION
			...
			$(caller).find("[class*=validate]").each(
					function()
					{
						linkTofield = $.validationEngine.linkTofield(this);

						if (!$("." + linkTofield).hasClass("ajaxed"))
						{ // DO NOT UPDATE ALREADY AJAXED FIELDS (only happen
							// if no normal errors, don't worry)
							var validationPass = $.validationEngine
									.loadValidation(this);
							return (validationPass) ? stopForm = true : "";
						}
						;
					});
			ajaxErrorLength = $.validationEngine.ajaxValidArray.length; // LOOK
																		// IF
																		// SOME
																		// AJAX
																		// IS
			...
			{ // GET IF THERE IS AN ERROR OR NOT FROM THIS VALIDATION
				// FUNCTIONS
				if ($.validationEngine.settings.animateSubmit)
				{
					destination = $(".formError:not('.greenPopup'):first")
							.offset().top;
